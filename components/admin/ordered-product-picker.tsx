...
export function OrderedProductPicker({
  ...
  function drop(targetId: string) {
    if (!draggingId || draggingId === targetId || pending) return;
    const source = orderedIds.find((id) => id === draggingId);
    if (!source) return;
  ...
  return (
    <div className={styles.stack}>
            ...
            return (
              ...
              >
                ...
                <div className={styles.rowActions}>
                  <button className={styles.textButton} disabled={index === 0 || pending} onClick={() => move(index, -1)} type="button">Subir</button>
                  <button className={styles.textButton} disabled={index === orderedIds.length - 1 || pending} onClick={() => move(index, 1)} type="button">Descer</button>
      ...
      <p className={status === "error" ? styles.formError : styles.fieldStatus} role="status">
        {status === "saving" ? "Salvando ordem..." : status === "saved" ? "Ordem salva." : status === "error" ? "Não foi possível salvar a ordem. Tente novamente." : "Arraste os produtos para reorganizar."}
      </p>
    </div>
  );
}
