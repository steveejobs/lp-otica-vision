...
begin
  ...
  select jsonb_build_object(
    ...
    'timeline',coalesce((select jsonb_agg(x order by x.day) from (select created_at::date day,count(*) filter(where normalized_name='page_view')::int page_views,count(*) filter(where normalized_name in ('product_whatsapp_clicked','general_whatsapp_clicked'))::int whatsapp from normalized group by 1) x),'[]'::jsonb),
