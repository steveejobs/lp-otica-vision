...
export default async function NewProductPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <>
      ...
      <section className={styles.formPanel} aria-labelledby="new-product-form"><h2 className={styles.eyebrow} id="new-product-form">Cadastro do produto</h2><ProductForm brands={brands} categories={categories} styleEligibilityReasons={["produto ainda não publicado", "sem capa publicada", "sem estilo ativo"]} styleOptions={styleResult.error ? [] : styleResult.data ?? []} /></section>
    </>
  );
}
