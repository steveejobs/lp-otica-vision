...
import { deleteGalleryAction, updateGalleryAction, uploadGalleryItemsAction } from "../actions";
...
export default async function EditGalleryPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; status?: string }> }) {
  ...
  const signed = await createAdminImageUrls("site-galleries", itemResult.data.map((item) => item.storage_path));
